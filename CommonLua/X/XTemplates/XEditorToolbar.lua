		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
					...
					'OnPress', function (self, gamepad)
						...
						if not toggle_back_visibility then
							for cat, filter in pairs(self.context.filter_buttons) do
								if filter == "invisible" then categories[cat] = true end
							end
						end
						XEditorFilters:ToggleFilter(categories, toggle_back_visibility)
						if LocalStorage.FilteredCategories["HideTop"] == "invisible" then XEditorFilters:UpdateVisibility("HideTop", "invisible") end
				...
				PlaceObj('XTemplateForEach', {
					'array', function (parent, context) return XEditorFilters.GetCategories() end,
					'condition', function (parent, context, item, i) return context.filter_buttons[item] and item ~= "All" end,
					'run_after', function (child, context, item, i, n, last)
						local text = item
						if #text > 8 then text = string.sub(text, 1, 6) .. "..." end
						child:SetText(text)
					...
					PlaceObj('XTemplateWindow', {
						...
						'OnPress', function (self, gamepad)
							...
							if terminal.IsKeyPressed(const.vkControl) then
								for cat, filter in pairs(self.context.filter_buttons) do
									if cat == category then XEditorFilters:UpdateVisibility(cat, "visible")
									elseif filter == "visible" then XEditorFilters:UpdateVisibility(cat, "unselectable") end
								end
							else
						...
						'OnAltPress', function (self, gamepad)
							...
							local category = self:GetRolloverText()
							if terminal.IsKeyPressed(const.vkControl) then
								local categories = {[category] = true}
				...
				PlaceObj('XTemplateWindow', {
					...
					'OnPress', function (self, gamepad)
						...
						for _, category in ipairs(allCategories) do
							if not table.find(table.keys(self.context.filter_buttons), category) then table.insert(categories, category) end
						end
				...
				PlaceObj('XTemplateWindow', {
					...
					'OnPress', function (self, gamepad)
						...
						local allCategories = XEditorFilters:GetCategories()
						for _, category in ipairs(allCategories) do
							if table.find(table.keys(self.context.filter_buttons), category) and category ~= "All" then table.insert(categories, category) end
						end
						CreateRealTimeThread(function()
							local categories = WaitListMultipleChoice(nil, categories, "Choose category / categories:")
				...
				PlaceObj('XTemplateWindow', {
					...
					'OnChange', function (self, check)
						...
						XEditorSettingsJustOpened = true -- to focus the filter edit box
					end,
				}),
				}),
			PlaceObj('XTemplateWindow', {
				...
				PlaceObj('XTemplateWindow', {
					...
					'RolloverTemplate', "XEditorToolbarRollover",
					'RolloverText', T(144816122915, --[[XTemplate XEditorToolbar RolloverText]] "Roofs"),
					'BorderWidth', 1,
					'BorderColor', RGBA(128, 128, 128, 255),
				...
				PlaceObj('XTemplateWindow', nil, {
					PlaceObj('XTemplateFunc', {
						'name', "Open(self, ...)",
						'func', function (self, ...)
							local edit = CreateNumberEditor(self, "idEdit",
								function(multiplier)
							...
							local floors = 0
							MapForEach("map", "Room", function(o) if o.floor > floors then floors = o.floor end end)
							...
							edit.OnShortcut = function(self, shortcut, source, ...)
