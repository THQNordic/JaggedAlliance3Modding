	...
	PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateWindow', {
			...
			'OnContextUpdate', function (self, context, ...)
				...
				if open then self:CloseCombo() end
				...
				if open then self:OpenCombo("select") end
			end,
			'Items', function (self) return XEditorUndo:GetOpNames() end,
		...
		PlaceObj('XTemplateWindow', {
			...
			'Show', "icon",
			'ButtonTemplate', "XEditorToolbarButton",
		...
		PlaceObj('XTemplateWindow', {
			...
			'Editable', true,
			'Items', function (self) return XEditorGroupsComboItems(editor.GetSel()) end,
			...
			'OnTextChanged', function (self, value)
				...
				CreateRealTimeThread(function()
					if WaitQuestion(terminal.desktop, Untranslated("Warning"),
							Untranslated(string.format("No such group '%s'. Create a new group?", value)),
							Untranslated("Yes"),
							Untranslated("No")) == "ok"
					then
						local groups = { value }
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
				...
				PlaceObj('XTemplateWindow', {
					...
					'Items', function (self) return XEditorSettings:GetSnapModes() end,
					'ArbitraryValue', false,
					'OnValueChanged', function (self, value)
						...
						if GetDialog("XSelectObjectsTool") or GetDialog("XPlaceObjectTool") then XEditorSettings:OnEditorSetProperty("SnapMode") end
			...
			'OnContextUpdate', function (self, context, ...)
				...
				if sel then
					...
					if editor_info and editor_info ~= info then
						...
						local max_len = 70
						if #info > max_len then
							info = string.sub(info, 1, max_len)
						end
					end
					text[#text + 1] = info .. (#editor.GetSel() > 1 and ", ..." or "")
				end
