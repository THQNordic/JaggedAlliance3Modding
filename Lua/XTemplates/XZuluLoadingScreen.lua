		...
		'OnContextUpdate', function (self, context, ...)
			if context.loaded then
				...
				if IsInMultiplayerGame() then
					self.idTimeoutLeave:SetVisible(true)
					self.idTimeoutLeave:SetEnabled(false)
					self.idTimeoutLeave:ActivateTimeout()
				end
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, ...)
				...
				if self.context.metadata and self.context.metadata.sector and (loadingReason == "load savegame" or loadingReason == "host game" or loadingReason == "zulu load savegame") then
					...
					sector.display_name = T{TranslationTable[self.context.metadata.mapName]}
				...
				local img_ext = ""
				if UIL.GetScreenSize():y()>=2160 then
					-- img_ext = ".4k" -- disabled 4K loading screens for now
				end
				local img = g_DefaultLoadingScreen
				local campaign_folder
				local sector_img
				...
				if next(data) and sector then
					...
					self.idSector:SetText(T{764093693143, "<SectorIdColored(id)>", id = data.sector})
					...
					if tod == "Sunrise" then tod = false end
					if tod == "Sunset" then tod = false end		
					if tod == "Day" then tod = false end			
				...
				if data and data.quest_tracker then
					for _, questData in ipairs(data.quest_tracker) do
						if questData.questVars then
							--Handle old data structure for quest tracker
							local notes = {}
							for _, noteTId in ipairs(questData.questNotes) do
								table.insert(notes, T{TranslationTable[noteTId], questData.questVars})
							end
							table.insert(quests, {Name = T{TranslationTable[questData.questName], questData.questVars}, Notes = notes})
						else
							local notes = {}
							for _, noteData in ipairs(questData.questNotes) do
								table.insert(notes, Untranslated(noteData) )
							end
							table.insert(quests, { Name = Untranslated(questData.questName), Notes = notes })
						end
					end
				...
				if self.idHint:GetText() ~= "" then
					...
					if weather_hint then
						hint = T{989146922853, "<DisplayName> - <description>", weatherPreset}
						seen_hints[weather] = true
					elseif night_hint then
						hint = T{989146922853, "<DisplayName> - <description>",todPreset}
						seen_hints[tod] = true
					else
						local rnd = AsyncRand(100)
						if rnd > 50 or (not  has_weather  and not has_night) then
							...
							if data and data.lethal_weapons then
								if hint.id == "Downed" then
									hint = Presets.LoadingScreenHint.Default[g_LoadingHintsNextIdx or 1]
									g_LoadingHintsNextIdx = (g_LoadingHintsNextIdx or 1) + 1
									if g_LoadingHintsNextIdx > #Presets.LoadingScreenHint.Default then
										g_LoadingHintsNextIdx = 1
							...
							if has_weather and has_night then
								local rnd  = AsyncRand(100)
								hint = rnd > 50  and T{989146922853, "<DisplayName> - <description>",weatherPreset} or T{989146922853, "<DisplayName> - <description>",todPreset}
								seen_hints[rnd > 50 and weather or tod] = true
							else
								hint = has_weather  and T{989146922853, "<DisplayName> - <description>",weatherPreset} or T{989146922853, "<DisplayName> - <description>",todPreset}
			...
			PlaceObj('XTemplateWindow', {
						...
						PlaceObj('XTemplateForEach', {
							...
							PlaceObj('XTemplateWindow', {
								...
								PlaceObj('XTemplateWindow', {
									...
									PlaceObj('XTemplateForEach', {
										'comment', "quest objective",
										'array', function (parent, context) return context.Notes end,
										'run_after', function (child, context, item, i, n, last)
										...
										PlaceObj('XTemplateWindow', {
											...
											'UseClipBox', false,
											'HandleMouse', false,
											'TextStyle', "PDAQuestTrackerDescr",
											'Translate', true,
										}),
										PlaceObj('XTemplateWindow', {
								...
								PlaceObj('XTemplateWindow', {
									...
									'MaxHeight', 1,
									'FoldWhenHidden', true,
									'Background', RGBA(130, 128, 120, 255),
									'Transparency', 77,
								}),
								}),
							}),
				...
				PlaceObj('XTemplateWindow', {
					...
					PlaceObj('XTemplateWindow', {
						...
						'Image', "UI/Inventory/T_Backpack_Slot_Small_2",
						'ImageFit', "stretch",
						'ImageRect', box(5, 0, 95, 100),
					}, {
						PlaceObj('XTemplateWindow', {
							'__class', "XText",
							'Id', "idHint",
							'Padding', box(30, 40, 2, 2),
							'HAlign', "left",
							'VAlign', "top",
							'MinWidth', 730,
							'MaxWidth', 730,
						...
						PlaceObj('XTemplateWindow', {
							...
							'Translate', true,
							'Text', T(734804923935, --[[XTemplate XZuluLoadingScreen Text]] "(dev)text set in ActivateTimeout function"),
						}, {
							PlaceObj('XTemplateWindow', {
								...
								'Translate', true,
								'Text', T(456661218767, --[[XTemplate XZuluLoadingScreen Text]] "<ButtonB>"),
								'TextVAlign', "center",
							}),
							PlaceObj('XTemplateFunc', {
								'name', "ActivateTimeout(self)",
								'func', function (self)
									CreateRealTimeThread(function()
										...
										self:SetText(T(287468350986, "Leave multiplayer"))
										self:SetEnabled(true)
										self.idControllerHint:SetVisible(GetUIStyleGamepad())
									end)
								end,
							}),
							}),
						PlaceObj('XTemplateWindow', {
							'__condition', function (parent, context) return not context.loaded end,
							'Id', "idLoading",
							'Margins', box(0, 0, 30, 0),
							'HAlign', "right",
							'VAlign', "center",
