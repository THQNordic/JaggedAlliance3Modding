...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
				...
				'OnContextUpdate', function (self, context, ...)
					...
					self.idSubtitle:SetText(operation.progress_sub_title and operation.progress_sub_title~="" and operation.progress_sub_title or T{250567545113, "in progress", operation})
					
					self.idTime:SetVisible(next(mercs))
					if next(mercs) then
						local left_time  = GetOperationTimeLeft(mercs[1], operation.id, {mercs = mercs ,prediction = true, all =true})
						self.idTime:SetText(T{795605774721, " <timeDuration(left_time)>", left_time = left_time})
					end					
					...
					if operation.id=="TrainMercs" then
						...
						self.idSubtitle:SetText(T{273655031960, "Training <stat>", stat=stat_name})
					end
				end,
			}, {
				PlaceObj('XTemplateWindow', {
					...
					'Translate', true,
					'Text', T(326409416767, --[[XTemplate SectorOperationDescrProgressUI Text]] "Operations"),
					'TextVAlign', "center",
				}),
				PlaceObj('XTemplateWindow', {
					'MinWidth', 366,
					...
					PlaceObj('XTemplateWindow', {
						...
						PlaceObj('XTemplateWindow', {
							...
							'__condition', function (parent, context) return next(context) end,
							'__class', "XContextWindow",
							...
							PlaceObj('XTemplateWindow', {
								...
								PlaceObj('XTemplateForEach', {
									...
									'run_after', function (child, context, item, i, n, last)
										...
										if item.resource=="Money" then return end
										
										local def = InventoryItemDefs[item.resource]
										child.RolloverTitle = T{def.DisplayName}
										child.RolloverText = T{def.Description or def.AdditionalHint}
									end,
								}, {
									PlaceObj('XTemplateWindow', {
										'__class', "XContextWindow",
						...
						PlaceObj('XTemplateWindow', {
							...
							'HAlign', "left",
							'VAlign', "center",
							'Image', "UI/SectorOperations/T_Icon_Activity_Resting",
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
				'__class', "ZuluFrameProgress",
				'Id', "idOperationProgress",
				'Margins', box(4, 0, 4, 0),
				'VAlign', "center",
				'MinHeight', 22,
				'MaxHeight', 22,
				...
				'OnContextUpdate', function (self, context, ...)
					...
					progress = max>0 and MulDivRound(operation:ProgressCurrent(next(mercs) and mercs[1], sector, "prediction") or 0, 100, max) or 0
					if operation.id=="RepairItems" then						
						max = next(mercs) and mercs[1].OperationInitialETA or 0
						if max > 0 then
							local current = max -  GetOperationTimerETA(mercs[1])--GetOperationTimeLeft(context, context.Operation)
		...
		PlaceObj('XTemplateWindow', {
			'__context', function (parent, context) return context.operation.image end,
			'__class', "XContextImage",
			'Id', "idActivityImage",
			'Margins', box(0, 18, 0, 0),
			'Dock', "top",
			'HAlign', "left",
			'VAlign', "top",
			'MinWidth', 368,
			...
			'MaxWidth', 368,
			'MaxHeight', 260,
			'Background', RGBA(189, 171, 149, 255),
			'ImageFit', "largest",
			'ContextUpdateOnOpen', true,
			'OnContextUpdate', function (self, context, ...)
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
				...
				'Translate', true,
				'Text', T(680437447125, --[[XTemplate SectorOperationDescrProgressUI Text]] "<description>"),
				'Shorten', true,
			}, {
