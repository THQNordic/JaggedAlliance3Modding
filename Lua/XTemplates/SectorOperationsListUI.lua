...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		'__context', function (parent, context) return {sector = context, operation = false} end,
		'__class', "XDialog",
		'Id', "idMain",
		'LayoutMethod', "HList",
		'HandleMouse', true,
			...
			PlaceObj('XTemplateFunc', {
				...
				'func', function (self, ...)
					...
					if rawget(node, "idStatsTable") then
						local txt_entries = {{text = T(555509617729, "Squads"),value = Untranslated(#squads)},{text = T(656958980161, "Mercs"),value = Untranslated(all_mercs)}}
						for k, v in sorted_pairs(profs) do
							txt_entries[#txt_entries + 1] = {text = v.display_name_plural, value = Untranslated(profs_count[k])}
						end
						txt_entries[#txt_entries +1] = {text = T(903415942850, "Others"),value = Untranslated( others_count)}
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
				'__context', function (parent, context) return GetOperationsInSector(context.sector.Id) end,
				'__class', "SnappingScrollArea",
				'Id', "idList",
				'Margins', box(38, 38, 38, 0),
				'LayoutMethod', "HWrap",
				'LayoutHSpacing', 32,
				'LayoutVSpacing', 42,
				...
				PlaceObj('XTemplateForEach', {
					...
					PlaceObj('XTemplateTemplate', {
						'__template', "SectorOperationButtonUI",
					}),
					}),
				}),
			PlaceObj('XTemplateWindow', {
