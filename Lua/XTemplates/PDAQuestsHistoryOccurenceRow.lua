...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateWindow', {
			...
			'MinWidth', 43,
			'Clip', "self",
			'TextStyle', "PDAQuests_HistoryTimestamp",
			...
			'OnContextUpdate', function (self, context, ...)
				...
				if preset and preset.sector then
					local text = T{preset.sector}
					self:SetText(text)
				elseif rowContext and rowContext.sector then
					local text = T{rowContext.sector}
					self:SetText(text)
				elseif rowContext and rowContext.sectorId then
					local text = T{877276211685, "<SectorId(sector)>", sector = rowContext.sectorId}
					self:SetText(text)
				end
