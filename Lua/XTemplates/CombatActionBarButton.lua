...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		...
		'RolloverAnchorId', "idBarHolder",
		'RolloverOffset', box(0, 0, 0, 55),
		'BorderWidth', 2,
		...
		PlaceObj('XTemplateWindow', {
			'comment', "Precalc data observer, updates action descriptions etc",
			'__context', function (parent, context) return "unit_precalc" end,
			'__class', "XContextWindow",
			'Id', "idPrecalcObserver",
			...
			'OnContextUpdate', function (self, context, ...)
				...
				elseif GetUIStyleGamepad() then
					...
					elseif underlyingAction.id == "Overwatch" then
						local tag = GetPlatformSpecificImageTag("RightTrigger", 800) .. GetPlatformSpecificImageTag("ButtonY", 650)
						tag = Untranslated(tag)
						button:SetText(tag)
					...
					local text
					local shortcuts
					-- Show the free aim shortcut on the default action if it will go into free aim.
					local defaultAction = unit:GetDefaultAttackAction()--("ranged")
					...
					if not text then
						-- Get display binding from action redirect
						...
						text =    GetShortcutButtonT(keybindActionName)
							    or GetShortcutButtonT("combatAction" .. combat_action.id)
								 or  T(460169163499, " ")
					end
					button:SetText(T{775518317251, "[<name>]",name = text})
				end
			end,
		}),
		PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, ...)
				...
				self.OnPressParam = combat_action.id -- The action is to be invoked
				self:SetId(underlyingAction.id) -- The button pretends to be the underlying action though.
		...
		PlaceObj('XTemplateFunc', {
			'name', "SetCurrentAction(self, sel)",
			'func', function (self, sel)
