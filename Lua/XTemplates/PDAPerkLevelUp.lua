...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		...
		'OnContextUpdate', function (self, context, ...)
			...
			self:SetRolloverTitle(T{perk.DisplayName, perk})
			self:SetRolloverText(T{perk.Description, perk})
			
			-- requirements
			if context[perk.Stat] < perk.StatValue then
				local text = T{646447383292, "Required <stat> <halign right><statValue>", stat = perk.Stat, statValue = perk.StatValue}
				--self:SetRequirementsText(text)
			end
		...
		'OnPress', function (self, gamepad)
			...
			if not HasPerk(perksDlg.unit, perkId) then -- not owned perk
				if self.Toggled then -- deselect it
					perksDlg:SelectPerk(perkId, false)
				else -- not yet selected
					if perksDlg.PerkPoints > 0 then -- select it if has points
						perksDlg:SelectPerk(perkId, true)
					else -- no more points
						if perksDlg.SelectedPerkIds and #perksDlg.SelectedPerkIds == 1 then -- only 1 point and a perk is selected
							local oldPerkId = perksDlg.SelectedPerkIds[1]
							perksDlg:SelectPerk(oldPerkId, false) -- deselect old perk
							if perksDlg:CanUnlockPerk(perksDlg.unit, CharacterEffectDefs[perkId]) then -- new perk is ok select it
								perksDlg:SelectPerk(perkId, true)
							else
								perksDlg:SelectPerk(oldPerkId, true) -- new perk cannot be selected without old. revert
						...
						else -- many points and all spent
		...
		PlaceObj('XTemplateWindow', {
			...
			'Dock', "box",
			'Visible', false,
			'Image', "UI/Inventory/perk_achieved",
			...
			PlaceObj('XTemplateWindow', {
				...
				'HAlign', "right",
				'VAlign', "bottom",
				'Image', "UI/Inventory/perk_achieved_marker",
			}),
			}),
		PlaceObj('XTemplateWindow', {
			'comment', "icon",
			...
			'DisabledBackground', RGBA(32, 35, 47, 255),
			'ImageScale', point(700, 700),
			'DisabledImageColor', RGBA(255, 255, 255, 78),
			'ContextUpdateOnOpen', true,
			'OnContextUpdate', function (self, context, ...)
	...
	PlaceObj('XTemplateProperty', {
		...
		'name', T(781869729879, --[[XTemplate PDAPerkLevelUp name]] "Perk Id"),
	}),
})
