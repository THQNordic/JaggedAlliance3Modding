...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateTemplate', {
				...
				PlaceObj('XTemplateWindow', {
					...
					'OnContextUpdate', function (self, context, ...)
						...
						if task.hideProgress then
							...
							local text = T{418643651117, "<current>/<required>", current = task.currentProgress, required = task.requiredProgress}
							self:SetText(text)
						end
					end,
					'Translate', true,
			...
			PlaceObj('XTemplateWindow', {
				...
				PlaceObj('XTemplateWindow', nil, {
					PlaceObj('XTemplateWindow', {
						...
						'OnContextUpdate', function (self, context, ...)
							...
							if unit then self:SetText(unit.Nick) end
						end,
						'Translate', true,
					}),
					PlaceObj('XTemplateWindow', {
				...
				PlaceObj('XTemplateWindow', {
					'comment', "description",
					'__class', "XText",
					'TextStyle', "CombatTask_Description",
					'ContextUpdateOnOpen', true,
					'OnContextUpdate', function (self, context, ...)
						local task = context
						local unit = g_Units[task.unitId]
						local text = T{task.description, unit}
						self:SetText(text)
						if task.state ~= "inProgress" then
		...
		PlaceObj('XTemplateFunc', {
			'name', "Animate(self, startTime)",
			'func', function (self, startTime)
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, ...)
				...
				if taskAnim then
					if GetPreciseTicks() - taskAnim.startTime < self.animPulseDuration + self.animHideDuration then
