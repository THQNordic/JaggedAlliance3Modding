		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateFunc', {
				...
				'func', function (self, ...)
					...
					--self.idText:SetText(control:GetRolloverText())
					...
					local remaining = T{366780543124, "Time Remaining: <right><style PDARolloverTextMedium><timeLeft></style>",
						timeLeft = FormatCampaignTime(timeLeft, "all")
					}
					local exactDue = T{361706829696, "Event Due: <right><style PDARolloverTextMedium><DateFormatted(t)> <time(t)></style>",
						t = dueTime
					}
					
					local multiEvents = self.context.otherEvents
					multiEvents = multiEvents and #multiEvents > 0
					if multiEvents then
					...
					self.idTime:SetText(T{888723273529, "<remaining><newline><left><exactDue>", remaining = remaining, exactDue = exactDue})
								
					if multiEvents then
						--self.idInputContainer:SetVisible(false)
						--return
					end
					
					local inputHint = T(467579936265, "View Event Sector")
					if GetUIStyleGamepad() then
						inputHint = T(693443355606, "<ButtonA> ") .. inputHint
					else
						inputHint = T(789893969947, "<image UI/Icons/left_click 1700> ") .. inputHint
					end
					...
					if eventData and not multiEvents then
						...
						if text then
							if GetUIStyleGamepad() then
								text = T(536051309476, "<ButtonX> ") .. text
							else
								text = T(670225106383, "<image UI/Icons/right_click 1700> ") .. text
							end
			...
			PlaceObj('XTemplateWindow', {
				...
				PlaceObj('XTemplateForEach', {
					'comment', "event data",
					'array', function (parent, context) return context.rolloverData end,
					...
					PlaceObj('XTemplateWindow', {
						...
						PlaceObj('XTemplateWindow', {
							'__condition', function (parent, context) return context and context.mercs and #context.mercs > 0 end,
							'Id', "idMercs",
							'Margins', box(0, 6, 0, 0),
							'LayoutMethod', "HWrap",
							'LayoutHSpacing', 8,
						...
						PlaceObj('XTemplateWindow', {
							'__condition', function (parent, context) return context and context.mercs and context.mercs.leftSide and context.mercs.rightSide end,
							'__class', "XContextWindow",
							'Id', "idMercs",
							'FoldWhenHidden', true,
							'Background', RGBA(32, 35, 47, 255),
							...
							PlaceObj('XTemplateWindow', {
								...
								PlaceObj('XTemplateWindow', {
									'comment', "left",
									'__context', function (parent, context) return context.mercs.leftSide end,
									'__class', "XContextWindow",
									'IdNode', true,
									'LayoutMethod', "VList",
									'LayoutHSpacing', 10,
									...
									PlaceObj('XTemplateWindow', {
										...
										PlaceObj('XTemplateForEach', {
											'array', function (parent, context) return context.List end,
											'__context', function (parent, context, item, i, n) return item end,
											'run_after', function (child, context, item, i, n, last)
												local unit = g_Classes[context]
								...
								PlaceObj('XTemplateWindow', {
									'comment', "left",
									'__context', function (parent, context) return context.mercs.rightSide end,
									'__class', "XContextWindow",
									'IdNode', true,
									'LayoutMethod', "VList",
									'LayoutHSpacing', 10,
