...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		'__context', function (parent, context) context.em = const.BlueEMColor return context end,
		'__class', "ConversationDialog",
		'ZOrder', 3,
		'HandleMouse', true,
	}, {
		PlaceObj('XTemplateFunc', {
			'name', "AnimPortrait(self, ctrl, hide)",
			'func', function (self, ctrl, hide)
		...
		PlaceObj('XTemplateFunc', {
			'name', "AnimBackImg(self, hide)",
			'func', function (self, hide)
				local duration = self:GetAnimDuration()
		...
		PlaceObj('XTemplateFunc', {
			'name', "GetAnimDuration(self, type)",
		...
		PlaceObj('XTemplateFunc', {
			'name', "PlayConversationLine(self, text, wait, sound_before, sound_after, sound_type,subline)",
			'func', function (self, text, wait, sound_before, sound_after, sound_type,subline)
				...
				-- reset Margins/MaxHeight adjustment made in SetupBackgroundSizeAndScrolling
				...
				if GetConvoSyncDataForState(self.player, 1) then
					return --line was already skipped
				end
		...
		PlaceObj('XTemplateFunc', {
			'name', "ClearKeywords(self, phrase_choices_available)",
			'func', function (self, phrase_choices_available)
				self.anim_hide = true
				...
				--self.idChoices:SetVisible(self.unit_template_id and phrase_choices_available, true)
				self.phrase_choices_available = phrase_choices_available
			end,
		}),
		PlaceObj('XTemplateFunc', {
			'name', "FillKeywords(self, position, keyword_data, nLeft, nRight)",
			'func', function (self, position, keyword_data, nLeft, nRight)
				local idxs
		...
		PlaceObj('XTemplateFunc', {
			'name', "SetDebugData(self, data, merc_id, conv_id)",
			'func', function (self, data, merc_id, conv_id)
				local btn = rawget(self, "idDebugButton")
				if btn then
					self.idDebugButton.OnPress = function(button)
		...
		PlaceObj('XTemplateFunc', {
			'name', "PhraseChoice(self, phrase)",
			'func', function (self, phrase)
				if self.window_state == "destroying" then return end
				if self.player.is_line_paused then return end
		...
		PlaceObj('XTemplateFunc', {
			'name', "WaitPhraseChoice(self, meta)",
			'func', function (self, meta)
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, pos, button)
				if button ~= "L" or not self:IsUIControllable() then return end
				if self.player.is_line_paused then
					NetSyncEvent("ConversationUnpause", self.player:GetState())
				end
				
				if not self.phrase_choices_available and self.window_state ~= "destroying" and
				   self.phrase_start_time and now() > self.phrase_start_time + const.UIButtonPressDelay
				then
					self:PhraseChoice()
					return "break"
				end
		...
		PlaceObj('XTemplateFunc', {
			'name', "OnShortcut(self, shortcut, source, controller_id, ...)",
			'func', function (self, shortcut, source, controller_id, ...)
				...
				local dpad_map = {
					...
					[2] =  {top = indexReplacements[1], down = indexReplacements[3] or indexReplacements[4] or indexReplacements[5], left = false, right = indexReplacements[5] or indexReplacements[4]},
					[3] =  {top = indexReplacements[2], down = indexReplacements[4] or indexReplacements[5],                         left = false, right = indexReplacements[6] or indexReplacements[4]},
					
					[11] = {top = false,                down = indexReplacements[3],                                                 left = false, right = indexReplacements[4]},
					[31] = {top = indexReplacements[1], down = indexReplacements[4] or indexReplacements[5],                         left = false, right = indexReplacements[6] or indexReplacements[4]},
				
					-- right
					[4] =  {top = indexReplacements[3] or indexReplacements[2],                         down = indexReplacements[5], left = indexReplacements[1],                         right = false},
					[5] =  {top = indexReplacements[4] or indexReplacements[3] or indexReplacements[2], down = indexReplacements[6], left = indexReplacements[2] or indexReplacements[1], right = false},
					[6] =  {top = indexReplacements[5],                                                 down = false,                left = indexReplacements[3] or indexReplacements[1], right = false},
					
					[41] = {top = indexReplacements[3] or indexReplacements[2],                         down = indexReplacements[6], left = indexReplacements[1] or indexReplacements[3], right = false},
				...
				local dpad_down  = XInput.IsCtrlButtonPressed(controller_id,"DPadDown")
				local dpad_left_up    = (dpad_left  and shortcut=="+DpadUp")   or (dpad_up   and shortcut=="+DPadLeft")
				...
				if not self.selected_phrase_idx then
					if shortcut=="+DPadUp" then
						self.selected_phrase_idx = prev and dpad_map[prev].top or last_idx or false
					elseif shortcut=="+DPadDown" then
						self.selected_phrase_idx = prev and dpad_map[prev].down or first_idx or false
					elseif shortcut=="+DPadLeft" then
				...
				if pressA then
					...
					if phrase:GetThread("effect") then
						--NetEchoEvent("SkipConversationTextAnimation") --this has no handler and would assert if it ever fired?
						return "break"
					end
					if self.player.is_line_paused then
						NetSyncEvent("ConversationUnpause", self.player:GetState())
		...
		PlaceObj('XTemplateFunc', {
			'name', "SetupBackgroundSizeAndScrolling(self, ctrl)",
			'func', function (self, ctrl)
				...
				-- if the text doesn't fit, try to fit it by cutting margins first (to prevent a scrollbar with a minimal scroll range)
				...
				if scroll_range > 0 and scroll_range <= max_margin_cut_y then
					...
					if new_margins ~= scroll_area.Margins then
						...
						return -- the function will be called again after a re-layout
					end
				end
		...
		PlaceObj('XTemplateFunc', {
			'name', "OnKbdKeyDown(self, vkey)",
			'func', function (self, vkey)
				...
				if not self.phrase_choices_available and self.window_state ~= "destroying" and
				   self.phrase_start_time and now() > self.phrase_start_time + const.UIButtonPressDelay
				then
					if vkey == const.vkSpace or vkey == const.vkEnter or vkey == const.vkEsc then
						if self.player.is_line_paused then
							NetSyncEvent("ConversationUnpause", self.player:GetState())
						end
						self:PhraseChoice()
		...
		PlaceObj('XTemplateFunc', {
			'name', "InitiatePhraseRolloverFadeout",
			'func', function (self, ...)
				local dlg = GetDialog(self)
				...
				-- the user can click with the mouse to hide the rollover immediately
				dlg.idPhraseRolloverFrame:SetHandleMouse(true)
				dlg.idPhraseRolloverFrame.OnMouseButtonDown = function(self, pt, button)
					if button == "L" then
						...
						self:SetVisible(false, not "instant", callback)
						return "break"
					end
				end
				
				-- hide the phrase "rollover" after 5 seconds as it may overlap with the phrase box
				dlg:CreateThread("FadeoutPhraseRollover", function()
					Sleep(5000)
					dlg.idPhraseRolloverFrame:SetFadeOutTime(3000)
					dlg.idPhraseRolloverFrame:SetVisible(false, not "instant", callback)
					-- the user can stop the rollover from fading by pointing it with the mouse
					dlg.idPhraseRolloverFrame.OnSetRollover = function(self, rollover)
						dlg.idPhraseRolloverFrame:SetFadeOutTime(1500)
						self:SetVisible(rollover, not "instant", not rollover and callback)
					end
				end)
			end,
		}),
		PlaceObj('XTemplateFunc', {
			'name', "StopPhraseRolloverFadeout",
			'func', function (self, ...)
				-- set default fade in/out times, kill previous thread (see code that creates the thread below)
				local dlg = GetDialog(self)
				dlg.idPhraseRolloverFrame:SetFadeOutTime(200)
				dlg.idPhraseRolloverFrame:SetHandleMouse(false)
				dlg.idPhraseRolloverFrame.OnSetRollover = nil
		...
		PlaceObj('XTemplateAction', {
			'comment', "activate Back or Goodbye keywords",
			'ActionId', "Close",
			'ActionName', T(963235061832, --[[XTemplate ConversationDialog ActionName]] "Close"),
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateFunc', {
				'name', "GetMouseTarget(self, pt)",
				'func', function (self, pt)
			...
			PlaceObj('XTemplateWindow', {
				...
				PlaceObj('XTemplateWindow', {
					...
					'HAlign', "left",
					'VAlign', "bottom",
					'Image', "CommonAssets/UI/Ged/help",
				}),
				}),
			PlaceObj('XTemplateWindow', {
				'HAlign', "center",
				...
				PlaceObj('XTemplateWindow', {
					...
					PlaceObj('XTemplateWindow', {
						...
						'Image', "UI/Mercs/Tex",
						'ImageRect', box(0, 0, 2000, 1500),
						'ImageScale', point(650, 650),
					}),
					PlaceObj('XTemplateWindow', {
						'__class', "XImage",
						'Id', "idGradient",
						'Margins', box(-1000, 450, -1000, 0),
						'Dock', "box",
						'VAlign', "bottom",
						'UseClipBox', false,
						'ChildrenHandleMouse', false,
						'Image', "UI/Conversation/Gradient",
						'ImageFit', "stretch-x",
					}),
					PlaceObj('XTemplateWindow', {
						'Margins', box(0, 0, 60, 40),
						'HAlign', "left",
						'VAlign', "bottom",
						'MinWidth', 1000,
						'MinHeight', 535,
						...
						PlaceObj('XTemplateWindow', {
							...
							PlaceObj('XTemplateWindow', nil, {
								...
								PlaceObj('XTemplateWindow', {
									...
									PlaceObj('XTemplateWindow', {
										'__class', "XFrame",
										'IdNode', false,
										'Margins', box(-8, -15, -8, -5),
										'HAlign', "right",
										'VAlign', "top",
										'UseClipBox', false,
										'Background', RGBA(195, 189, 172, 255),
										'FocusedBackground', RGBA(255, 255, 255, 0),
										'Image', "UI/Conversation/T_Choice_Rollover_Background",
									}, {
										PlaceObj('XTemplateWindow', {
											'__class', "XText",
											'Id', "idOtherPlayerText",
											'UseClipBox', false,
											'FoldWhenHidden', true,
											'FadeInTime', 200,
											'FadeOutTime', 200,
											'HandleKeyboard', false,
											'HandleMouse', false,
											'TextStyle', "ConversationCondition",
											'Translate', true,
											'HideOnEmpty', true,
											'TextHAlign', "right",
											'TextVAlign', "center",
							...
							PlaceObj('XTemplateWindow', {
								...
								PlaceObj('XTemplateWindow', {
									...
									PlaceObj('XTemplateWindow', {
										...
										'FadeOutTime', 100,
										'HandleMouse', false,
										'TextStyle', "ConversationName",
										'Translate', true,
									}),
									PlaceObj('XTemplateWindow', {
										'__class', "XImage",
										'Id', "idRadioImage",
										'Margins', box(-50, -50, 0, 0),
										'Dock', "left",
										'MaxHeight', 10,
										'Visible', false,
										'FadeInTime', 100,
								...
								PlaceObj('XTemplateWindow', {
									'__class', "XFrame",
									'Id', "idUndertitleImage",
									'Margins', box(0, 10, 0, 8),
									'Visible', false,
							...
							PlaceObj('XTemplateWindow', {
								...
								'LayoutMethod', "VList",
								'VScroll', "idScrollbar",
								'ScrollInterpolationTime', 250,
							}, {
								PlaceObj('XTemplateWindow', {
									'__context', function (parent, context) context.em = "<color 196 175 117>" return context end,
									'__class', "XText",
									'Id', "idPhrase",
									'Padding', box(2, 0, 2, 2),
									'Clip', false,
									...
									'FadeOutTime', 100,
									'HandleMouse', false,
									'TextStyle', "ConversationPhrase",
									'Translate', true,
								}),
								PlaceObj('XTemplateWindow', {
									'Id', "idInterjectionContainer",
									'LayoutMethod', "VList",
									'LayoutVSpacing', -5,
									'UseClipBox', false,
								}),
						...
						PlaceObj('XTemplateWindow', {
							'comment', "Positioned in parent's OnLayoutComplete",
							'__class', "XControl",
							'Id', "idParticles",
							'Dock', "ignore",
							'HandleKeyboard', false,
						...
						PlaceObj('XTemplateWindow', {
							...
							PlaceObj('XTemplateWindow', {
								...
								'FadeOutTime', 200,
								'FocusedBackground', RGBA(255, 255, 255, 0),
								'DisabledBackground', RGBA(80, 80, 75, 255),
							}, {
								PlaceObj('XTemplateWindow', {
									'__class', "XText",
									'Id', "idKeywordRollover",
									'UseClipBox', false,
									'FadeOutTime', 200,
									'HandleKeyboard', false,
									'HandleMouse', false,
									'TextStyle', "ConversationChoiceRollover",
									'Translate', true,
									'HideOnEmpty', true,
									'TextHAlign', "center",
									'TextVAlign', "center",
								...
								PlaceObj('XTemplateFunc', {
									'name', "UpdateAnimation(self, kwVisible, phVisible, instant)",
									'func', function (self, kwVisible, phVisible, instant)
										...
										self:CreateThread("frame_animation", function()
											...
											if rawget(self, "unfolded") == bgUnfolded then return end
											rawset(self, "unfolded", bgUnfolded)
											self:SetVisible(true) -- Always visible, but sometimes folded by interpolation
								...
								PlaceObj('XTemplateWindow', nil, {
									PlaceObj('XTemplateWindow', {
										...
										PlaceObj('XTemplateWindow', {
											...
											'UseClipBox', false,
											'Visible', false,
											'Image', "UI/Conversation/T_Dialogue_Arrow",
										}, {
											PlaceObj('XTemplateFunc', {
												'name', "PointTo(self, angle)",
												'func', function (self, angle)
											...
											PlaceObj('XTemplateFunc', {
												'name', "LockRotation(self, lock)",
												'func', function (self, lock)
													rawset(self, "lock", lock)
												end,
											}),
											PlaceObj('XTemplateFunc', {
												'name', "StartFollowThread(self)",
												'func', function (self)
													local dlg = GetDialog(self)
											...
											PlaceObj('XTemplateFunc', {
												...
												'func', function (self, ...)
													...
													local dpad_down  = state.DPadDown
													
													--print("left", dpad_left, "right",dpad_right, "up",dpad_up, "down", dpad_down)
													
													if dpad_left and dpad_up then
														return	315*60
													elseif dpad_left  and dpad_down then
														return 225*60					
													elseif dpad_right and dpad_up then
														return 45*60
													elseif dpad_right and dpad_down then
														return 135*60
													elseif dpad_left then
														return 270*60
													elseif dpad_right then
														return 90*60
													elseif dpad_up then
														return 0*60
													elseif dpad_down then
														return 180*60
													end	
													return 0
												end,
										...
										PlaceObj('XTemplateWindow', {
											...
											PlaceObj('XTemplateWindow', {
												...
												'HAlign', "center",
												'VAlign', "center",
												'Image', "UI/Conversation/T_Wheel_InnerCircle",
											}),
											PlaceObj('XTemplateWindow', {
												'__class', "XImage",
												'Id', "idStoryBranch",
												...
												'VAlign', "center",
												'Visible', false,
												'Image', "UI/Conversation/conversation_threaten",
												'DisabledImageColor', RGBA(80, 80, 75, 255),
											}),
											}),
										}),
									PlaceObj('XTemplateWindow', {
										...
										PlaceObj('XTemplateFunc', {
											'name', "OnButtonRollover(self, child, rollover, flip, pos)",
											'func', function (self, child, rollover, flip, pos)
										...
										PlaceObj('XTemplateWindow', {
											...
											PlaceObj('XTemplateWindow', {
												...
												PlaceObj('XTemplateTemplate', {
													'__template', "ActionButtonConversation",
													...
													'ConversationRolloverText', T(--[[XTemplate ConversationDialog ConversationRolloverText]] nil),
													'Align', "left",
												}, {
													PlaceObj('XTemplateFunc', {
							...
							PlaceObj('XTemplateWindow', {
								...
								'OnContextUpdate', function (self, context, ...)
									if GetUIStyleGamepad() then
										self:SetText(T(103942894433, "<ButtonA> Next"))
									else	
										self:SetText(T(240345008948, "<left_click> Next"))
									end
								end,
								'Translate', true,
