	...
	PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateForEach', {
				'comment', "give",
				'array', function (parent, context) return  context.action == "give" and InventoryGetTargetsForGiveAction(context) or  {} end,
				'condition', function (parent, context, item, i) return context.action == "give" end,
				'run_after', function (child, context, item, i, n, last)
					if type(item)=="number" then -- squad bag
			...
			PlaceObj('XTemplateForEach', {
				...
				'array', function (parent, context) return context.action == "giveToSquad" and InventoryGetTargetsForGiveToSquadAction(context) or  {} end,
				'condition', function (parent, context, item, i) return not InventoryIsCombatMode() and context.action == "giveToSquad" end,
				'run_after', function (child, context, item, i, n, last)
			...
			PlaceObj('XTemplateForEach', {
				'comment', "split and give merc",
				'array', function (parent, context) return context.action == "splitgiveToSquad" and  InventoryGetTargetsForGiveAction(context) or  {} end,
				'condition', function (parent, context, item, i) return not InventoryIsCombatMode() and context.action == "splitgiveToSquad" end,
				'run_after', function (child, context, item, i, n, last)
					local unit
					if gv_SatelliteView then
						unit = gv_UnitData[item]
			...
			PlaceObj('XTemplateForEach', {
				'comment', "split and give squad",
				'array', function (parent, context) return context.action == "splitgiveToSquad" and InventoryGetTargetsForGiveToSquadAction(context) or  {} end,
				'condition', function (parent, context, item, i) return not InventoryIsCombatMode() and context.action == "splitgiveToSquad" end,
				'run_after', function (child, context, item, i, n, last)
					child:SetText(Untranslated(item.Name))
					child.squad = item
			...
			PlaceObj('XTemplateForEach', {
				'comment', "combine",
				'array', function (parent, context) return context.action == "combine" and InventoryGetTargetsRecipe(context.item, context.unit) or {} end,
				'condition', function (parent, context, item, i) return context.action == "combine" end,
				'item_in_context', "recipe_item",
				'run_after', function (child, context, item, i, n, last)
					...
					if amount>1 then
						child:SetText(T{736794276226, "+<name>(<amount>) = <result>(<result_amount>)",name =  name, amount=amount, result = result_name, result_amount =  result_amount})
					else
						child:SetText(T{619588210566, "+<name><equipped> = <result>",name =  name,  result = result_name, equipped = IsEquipSlot(container_slot) and T{678824253741, "(Equipped-<Nick>)",item.container_data.container} or ""})
					end
					child:SetFocusOrder(point(2, i))
				end,
			}, {
				PlaceObj('XTemplateTemplate', {
					...
					'OnPress', function (self, gamepad)
						...
						local slot = context.slot_wnd
						local item =  context.item								
						local recipe_data = self.context.recipe_item
						local recipe = recipe_data.recipe
						local cd = recipe_data.container_data
						CombineItemsLocal(recipe, context.unit, item, context.context, cd.item, cd.container, self.desktop)
						slot:ClosePopup()]]
					end,
				}),
				}),
			PlaceObj('XTemplateForEach', {
				'comment', "use",
				'array', function (parent, context) return context.action == "use" and InventoryGetSquadUnits() or {} end,
				'condition', function (parent, context, item, i) return context.action == "use" end,
				'run_after', function (child, context, item, i, n, last)
					local unit 
					if gv_SatelliteView then
						unit = gv_UnitData[item]
			...
			PlaceObj('XTemplateForEach', {
				'comment', "ammo (reload action)",
				'array', function (parent, context) return context.action == "reload" and GetReloadOptionsForWeapon(context.item, context.context) or {} end,
				'condition', function (parent, context, item, i) return context.action == "reload" end,
				'run_after', function (child, context, item, i, n, last)
					child:SetContext(item)
					local count = context.unit:CountAvailableAmmo(item.ammo.class)
					child:SetText(T{433225540474, "<ammo_type>(<count>)", ammo_type = item.ammo.DisplayName, count = count})
					child:SetFocusOrder(point(2, i))
					
					if not IsWeaponAvailableForReload(item.weapon, {item.ammo}) then
				...
				PlaceObj('XTemplateTemplate', {
					...
					'Text', T(231508638088, --[[XTemplate InventoryContextSubMenu Text]] "Ammo"),
				}),
				}),
			}),
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
				...
				'Translate', true,
				'Text', T(404959391934, --[[XTemplate InventoryContextSubMenu Text]] "OPTIONS"),
				'TextVAlign', "center",
			}),
			}),
		PlaceObj('XTemplateFunc', {
