	...
	PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateWindow', {
			...
			'OnContextUpdate', function (self, context, ...)
				...
				local rollover_hint = context[1]:GetRolloverHint()
				local text = ""
				
				self.idTitle:SetText(T{412158497796, "<amount> x <name>",name = context.DisplayName,  amount = item.amount})
				if context.Description and context.Description ~= "" then
					text = text .. context.Description
				end
				if rollover_hint and rollover_hint ~= "" then
					text = text .. "\n\n" .. rollover_hint
				end
				if text ~= "" then
			...
			PlaceObj('XTemplateWindow', {
				...
				PlaceObj('XTemplateWindow', {
					...
					'Translate', true,
					'Text', T(501701431103, --[[XTemplate RolloverOperationCraftRecipe Text]] "Ingredients"),
					'TextVAlign', "bottom",
				}),
				PlaceObj('XTemplateForEach', {
					'array', function (parent, context) return CraftOperationsRecipes[context.control.item.recipe].Ingredients end,
					'run_after', function (child, context, item, i, n, last)
						...
						if all then
							...
							if not result then
								child.idAmount:SetText(T{322717151656, "<style InventoryItemsCount><GameColorI><cur></color><valign bottom 0><style InventoryItemsCountMax>/<max></style>", 
									cur = amount_found, max = item.amount})
							else
							child.idAmount:SetText(T{709831548750, "<style InventoryItemsCount><cur><valign bottom 0><style InventoryItemsCountMax>/<max></style>", 
								cur = item.amount, max = item.amount})
							end
						else
							child.idAmount:SetText(T{127840827053, "<style InventoryItemsCount><cur></style>", 
								cur = item.amount})
						end
					end,
				}, {
