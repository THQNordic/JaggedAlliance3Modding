...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		'__class', "TermClarifyingRollover",
		'BorderWidth', 0,
		'MaxWidth', 40000,
		'LayoutMethod', "HList",
		...
		'OnContextUpdate', function (self, context, ...)
			...
			self.idContent.idMoreInfo:SetVisible(terms and #terms > 0, "terms")
		end,
	}, {
		PlaceObj('XTemplateWindow', {
			...
			'OnContextUpdate', function (self, context, ...)
				...
				if not context.action then return end
				...
				if apCostText == "" then
					...
					apCostText = T{apCostText, ap = apCostMin, apm = apCostMax}
				
					self.idAPCostText:SetText(T{569524074495, "<apCostText><valign bottom><style CombatActionRolloverAP>/<apn(unitAp)> AP</style>",
				...
				if text ~= "placeholder" then -- We need a dummy value otherwise the rollover won't open.
				...
				if action and (not hintText or hintText=="") then
					...
					elseif action.AimType == "melee" then
						...
						if not use_action:GetAnyTarget(Selection) then
							if use_action == action then
								hintText = T(477584135912, "<error>No enemies in melee range</error>")
							else
								-- mark target (free aim maybe)
							end
						end
					end
				end
				...
				if action and (action.group == "WeaponAttacks" or action.group == "FiringModeMetaAction") then
					...
					-- Try to get data from damage prediction (it's more accurate) if in the crosshair.
					...
					if (action.id == "LeftHandShot" or action.id == "RightHandShot") and not self.idWeaponShow then
						...
						ammoText:SetText(T{414344497801, "<bullets()>", wep})
					...
					if wep2 then					
						...
						if w1Shots > 1 then
							self.idDamage:SetText(T{707333089408, "<style PDARolloverHeaderBeige><shots>x<damage></style> <valign bottom -2>DMG", shots = w1Shots, damage = params.wep1_base})
							self.idDamage2:SetText(T{707333089408, "<style PDARolloverHeaderBeige><shots>x<damage></style> <valign bottom -2>DMG", shots = w2Shots, damage = params.wep2_base})
						else
							self.idDamage:SetText(T{370219953309, "<style PDARolloverHeaderBeige><damage></style> <valign bottom -2>DMG", damage = w1Damage})
							self.idDamage2:SetText(T{370219953309, "<style PDARolloverHeaderBeige><damage></style> <valign bottom -2>DMG", damage = w2Damage})
						end
					...
					elseif aoeDamage > 0 then
						...
						self.idDamage:SetText(T{370219953309, "<style PDARolloverHeaderBeige><damage></style> <valign bottom -2>DMG", damage = damage})
					end
				elseif action and action.ActionType == "Ranged Attack" then -- Non "WeaponAttacks" attacks, such as consumables and signature skills
					local wep, wep2 = action:GetAttackWeapons(SelectedObj)
					if IsKindOf(wep, "Grenade") then
						self.idAttackInfoSection:SetVisible(true)
					
						local function lSetTextFromProperty(textWnd, propertyPreset)
							local presetShortName = propertyPreset.short_display_name
							local value = propertyPreset:GetProp(wep, SelectedObj.session_id)
							textWnd:SetText(T{943797174493, "<style PDARolloverHeaderBeige><value></style> <valign bottom -2><shortName>", value = value, shortName = presetShortName})
						end
