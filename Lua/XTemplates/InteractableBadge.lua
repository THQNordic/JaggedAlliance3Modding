...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateWindow', {
			...
			'MouseCursor', "UI/Cursors/Interact.tga",
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, rollover)
				if self.idImage.Image ~= "UI/Hud/iw_loot" then return end
							
				local function lGetBobInterp(b)
					return {
						id = "bob",
						type = const.intRect,
						duration = 1000,
				...
				if rollover then
					...
					elseif not self:GetThread("bob-when-ready") then
						...
						self:CreateThread("bob-when-ready", function()
							WaitNextFrame(1)
							local myBox = self.box
							if myBox ~= empty_box then -- You had your chance
								self:AddInterpolation(lGetBobInterp(myBox))
							end
						end)
					end
