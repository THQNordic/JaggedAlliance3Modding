...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateForEach', {
		'comment', "CombatActions -> XActions",
		'array', function (parent, context) return PresetMultipleGroupsCombo("CombatAction", {"UIActions", "Hidden", "Stances"}, false, "no_empty")() end,
		'condition', function (parent, context, item, i) return CombatActions[item].ShowIn end,
		'run_after', function (child, context, item, i, n, last)
			...
			if action_bindings then -- override empty or shortcut2
			...
			child:SetActionShortcuts(shortcut, shortcut2, shortcut_gamepad)
			StripDeveloperShortcuts(child)
		end,
	}, {
		PlaceObj('XTemplateAction', {
			'ActionState', function (self, host)
				if #Selection == 0 then return "disabled" end
				local action = CombatActions[self.ActionId]
				if not action then return "hidden" end
				local selObj = Selection[1]
				local underlying = action:ResolveAction(selObj) or action
				
				-- Actions appearing in "CombatActions" are checked by RecalcUIActions
	...
	PlaceObj('XTemplateForEach', {
		'comment', "reload actions",
		'array', function (parent, context) return PresetGroupCombo("InventoryItemCompositeDef", "Ammo")() end,
		'run_after', function (child, context, item, i, n, last)
			child.ActionId = item
