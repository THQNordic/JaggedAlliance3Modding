...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		...
		'OnPress', function (self, gamepad)
			...
			dlg:SetMode("Operation", {operation = self.context.id})
		end,
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, rollover)
				...
				local dlg = GetDialog(self)
				if dlg.window_state == "destroying" then return end
				...
				if sector.started_operations and sector.started_operations[ac.id] then
					...
					if idx then
						...
						local icon --= g_SatTimelineUI.icons_created[idx]
				...
				if rollover  then
					dlg:SetContext({sector = dlg.context.sector, operation = ac})
					dlg.idOperationDescr:SetContext({sector = dlg.context, operation = ac})
					dlg.idOperationDescr:ResolveId("idOperationDescrText"):SetText(ac.description)
				else
					dlg:SetContext({sector = dlg.context.sector, operation = false})
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
				...
				'MaxWidth', 36,
				'MaxHeight', 36,
				'Image', "UI/SectorOperations/T_Activities_Icon_New",
			}),
			PlaceObj('XTemplateWindow', {
				'__class', "XImage",
				'Id', "idQuest",
			...
			PlaceObj('XTemplateWindow', {
				...
				PlaceObj('XTemplateWindow', {
					...
					'OnContextUpdate', function (self, context, ...)
						...
						progress = max>0 and MulDivRound(context:ProgressCurrent(mercs[1], sector, "prediction") or 0, 100, max) 
						if context.id=="RepairItems" then
							...
							if max > 0 then
								local current = max -  GetOperationTimerETA(mercs[1], "prediction")--GetOperationTimeLeft(context, context.Operation)
