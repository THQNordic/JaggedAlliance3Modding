...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		...
		PlaceObj('XTemplateWindow', {
			'__class', "XImage",
			'Id', "idExpandBackground",
			'Margins', box(0, 12, 15, 12),
			'Dock', "right",
			'MinWidth', 40,
			'MaxWidth', 40,
			'Background', RGBA(124, 130, 96, 64),
		}, {
			PlaceObj('XTemplateWindow', {
				'__class', "XImage",
				'Id', "idExpandArrow",
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, context, prop_meta, value)
				...
				elseif prop_meta.id == "Resolution" then
					text = T{664014484626, "<FormatResolution(pt)>", pt = value }
				else
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, pos, button)
				...
				if button == "L" then
					self:SetRolloverText("")--hide tootip info when clicked
					local dialog = GetDialog(self):ResolveId("idSubSubContent")
					self:OnValueChanged(self)
					if dialog and GetDialogModeParam(dialog) and 
					GetDialogModeParam(dialog).prop_meta.id == self.context.prop_meta.id and not GetUIStyleGamepad() then
						self:SetFocus(false)
						return "break"
					else
		...
		PlaceObj('XTemplateFunc', {
			...
			'func', function (self, rollover)
				...
				if rollover then 
					if not self.isExpanded then SetupOptionRollover(self, self.context[1], self.context.prop_meta) end
	...
	PlaceObj('XTemplateProperty', {
		...
		'editor', "text",
		'translate', false,
		'name', T(984243297590, --[[XTemplate PropChoice name]] "Message"),
		'help', T(575642562823, --[[XTemplate PropChoice help]] "Set message to be sent on value set."),
	}),
})
