...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		'__class', "PDAQuestsClass",
		'Id', "idQuestsContent",
		'Margins', box(16, 16, 6, 16),
		'HostInParent', true,
	}, {
		PlaceObj('XTemplateFunc', {
			'name', "GetMouseTarget(self, pt)",
			'func', function (self, pt)
				-- Hack for mouse detection of the sector id window that is sticking out in some cases.
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
				...
				'Id', "idQuestListContainer",
			}, {
				PlaceObj('XTemplateWindow', {
					'__class', "MessengerScrollbar",
					'Id', "idQuestScrollbar",
					'Dock', "right",
					'Target', "idQuestScroll",
					'AutoHide', true,
					'UnscaledWidth', 15,
				}),
				PlaceObj('XTemplateWindow', {
					...
					'Id', "idQuestScroll",
					'Margins', box(8, 5, 8, 10),
					'Dock', "box",
					'VScroll', "idQuestScrollbar",
					...
					PlaceObj('XTemplateForEach', {
						'comment', "section",
						'array', function (parent, context) return GetDialog(parent).sections end,
						'condition', function (parent, context, item, i) return #item.Content > 0 end,
						...
						PlaceObj('XTemplateForEach', {
							'comment', "quest header",
							'array', function (parent, context) return context.Content end,
							'condition', function (parent, context, item, i) return item.questHeader end,
					...
					PlaceObj('XTemplateFunc', {
						'name', "EnactHideKey(self, key, value)",
		...
		PlaceObj('XTemplateWindow', {
			'comment', "selected quest info",
			'Margins', box(25, 0, 0, 0),
			'Dock', "box",
		}, {
			PlaceObj('XTemplateWindow', {
				'__context', function (parent, context) return "selected_quest" end,
				'__class', "XContentTemplate",
				'Id', "idSelectedQuestContainer",
			}, {
				PlaceObj('XTemplateWindow', {
					'comment', "quest changes observer",
					'__context', function (parent, context) return gv_Quests end,
					'__class', "XContextWindow",
				...
				PlaceObj('XTemplateWindow', {
					'__context', function (parent, context) return GetDialog(parent):GetSelectedQuestData() end,
					...
					PlaceObj('XTemplateWindow', {
						...
						PlaceObj('XTemplateWindow', {
							...
							PlaceObj('XTemplateWindow', {
								...
								PlaceObj('XTemplateWindow', {
									...
									PlaceObj('XTemplateWindow', {
										...
										'HAlign', "center",
										'VAlign', "center",
										'Image', "UI/PDA/Quest/quest_main_",
									}),
									PlaceObj('XTemplateWindow', {
										'__class', "XImage",
										'HAlign', "center",
								...
								PlaceObj('XTemplateWindow', {
									...
									'MinHeight', 40,
									'MaxHeight', 40,
									'Background', RGBA(88, 92, 68, 125),
								}, {
									PlaceObj('XTemplateWindow', {
										'__context', function (parent, context) return context.questHeader.questPreset end,
										'__class', "XText",
										'Margins', box(2, 0, 0, 0),
										'VAlign', "bottom",
										'TextStyle', "PDAQuestTitleInfo",
										'ContextUpdateOnOpen', true,
										'OnContextUpdate', function (self, context, ...)
									...
									PlaceObj('XTemplateWindow', {
										...
										'HAlign', "right",
										'VAlign', "bottom",
										'TextStyle', "PDAQuestsNoteText",
										'ContextUpdateOnOpen', true,
										'OnContextUpdate', function (self, context, ...)
											if Platform.developer then
												self:SetText(T{Untranslated("(author: <author>)     <month(t)> <day(t)> <year(t)>"), t = context.earliestTimestamp, author = Untranslated(context.questHeader.questPreset.Author or "<color 255 0 0>Unknown</color>")})
											else
												self:SetText(T{147546503736, "<month(t)> <day(t)> <year(t)>", t = context.earliestTimestamp})
											end
											return XContextControl.OnContextUpdate(self, context)
										end,
										'Translate', true,
							...
							PlaceObj('XTemplateWindow', {
								'comment', "line below header",
								'Margins', box(16, 3, 0, 0),
								'VAlign', "top",
								'MinHeight', 1,
								'MaxHeight', 1,
						...
						PlaceObj('XTemplateWindow', {
							'comment', "notes container",
							'__class', "XScrollArea",
							'Id', "idNotesScrollArea",
							'IdNode', false,
							'Dock', "box",
							'LayoutMethod', "VList",
							'VScroll', "idNotesScroll",
						}, {
							PlaceObj('XTemplateWindow', {
								'comment', "placeholder",
							...
							PlaceObj('XTemplateForEach', {
								'array', function (parent, context) return context.questNotes end,
								'__context', function (parent, context, item, i, n) return item end,
								'run_after', function (child, context, item, i, n, last)
									local notePreset = context.preset
									local questPreset = context.questPreset
									child.idText:SetText(T{notePreset.Text, questPreset})
									...
									elseif context.sectors and #context.sectors > 0 then
										if #context.sectors > 1 then
											...
											child.idSectorId:SetText(T{933855793737, "<underline><SectorIdColored2(sectorId)></underline>", sectorId = context.sectors[1]})
										end
										child.idUnreadPh:SetText(child.idSectorId.Text)
									else
										-- It should take up the same space though :)
										child.idSectorId:SetText(Untranslated("B9"))
										child.idSectorId:SetVisible(false)
										child.idSectorIdContainer:SetHandleMouse(false)
									end
									...
									-- Modify quest data reference so the quest header can read it.
									context.read = true
								...
								PlaceObj('XTemplateWindow', {
									...
									PlaceObj('XTemplateWindow', {
											...
											'OnPress', function (self, gamepad)
												...
												CreateRealTimeThread(function()
													...
													if not gv_SatelliteView then
														...
														Sleep(1) -- Wait for layout and stuff
													end
											...
											PlaceObj('XTemplateWindow', {
												...
												PlaceObj('XTemplateFunc', {
													...
													'func', function (self, clip_box)
														-- Clip hack - override clipbox of scroll area
														return XText.DrawWindow(self, box(0, 0, max_int, max_int))
													end,
												}),
												}),
											PlaceObj('XTemplateFunc', {
												'name', "PointInWindow(self, pt)",
												'func', function (self, pt)
													if XButton.PointInWindow(self, pt) then return true end
													if not self.enabled then return false end
													
													local node = self:ResolveId("node")
													local sectorIdWin = node.idSectorId
													if not sectorIdWin.visible then return false end
										...
										PlaceObj('XTemplateWindow', {
											...
											'Id', "idCompleted",
											'Visible', false,
											'Image', "UI/PDA/Quest/checkmark",
										}),
										}),
									PlaceObj('XTemplateWindow', {
										'Id', "idUnreadContainer",
										'Dock', "ignore",
										'LayoutMethod', "HList",
									}, {
										PlaceObj('XTemplateTemplate', {
											'__template', "PDAQuestUnreadIndicator",
											'Margins', box(0, -5, 0, 0),
											'VAlign', "top",
											'MinWidth', 10,
											'MaxWidth', 10,
		...
		PlaceObj('XTemplateAction', {
			'ActionId', "SetSelectedQuestAsActive",
			...
			'ActionState', function (self, host)
				...
				-- Dont allow setting completed as active
				local isAvailable = data.questHeader.state ~= "completed" and data.questHeader.state ~= "failed"
				if not isAvailable then return "disabled" end
				
				-- If already active disable button
			...
			'OnAction', function (self, host, source, ...)
				...
				local selectedQuest = tasksDlg.selected_quest
				if selectedQuest then SetActiveQuest(selectedQuest) end
			end,
		}),
		PlaceObj('XTemplateAction', {
			'ActionId', "ShowHideCompleted",
			'ActionName', T(848609246777, --[[XTemplate PDAQuests_Tasks ActionName]] "<ToggleCompletedQuestsActionName()>"),
			'ActionToolbar', "ActionBar",
			'ActionShortcut', "S",
		...
		PlaceObj('XTemplateWindow', {
			'comment', "observes ^ show/hide completed",
			'__context', function (parent, context) return "quest_list_respawn" end,
			'__class', "XContextWindow",
			'OnContextUpdate', function (self, context, ...)
