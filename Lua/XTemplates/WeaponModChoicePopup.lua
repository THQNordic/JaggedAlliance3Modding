...
PlaceObj('XTemplate', {
	...
	PlaceObj('XTemplateWindow', {
		'__class', "WeaponModChoicePopupClass",
		'Id', "idChoicePopup",
		...
		PlaceObj('XTemplateWindow', {
			...
			PlaceObj('XTemplateWindow', {
					...
					PlaceObj('XTemplateWindow', {
						...
						'OnContextUpdate', function (self, context, ...)
							...
							for costType, amount in sorted_pairs(costs) do
								...
								if affordable then canAffordNone = false end
								
								local costContainer, costText = lSpawnCostText()
								costText:SetText(T{247376794745, "<name><right><style PDARolloverTextBold><amount></style>",
									name = name,
									amount = amount
								})
							...
							local difficultyTextRight = T{475358187496, "<ModificationDifficultyToText(difficulty, mercSkill)>", difficulty = difficulty, mercSkill = bestMercSkill}
							bestMerc = gv_UnitData[bestMerc]
							local skillTextLeft = T{478471305383, "Mechanical (<mercName>)", mercName = bestMerc:GetDisplayName()}
							local skillTextRight = Untranslated(bestMercSkill)
							
							-- LeftRight window is weird, just concat with tags
							local diffRow = T{480414943560, "<style PDARolloverTextDark><diffL></style><right><style PDARolloverText><diffR></style>", diffL = difficultyTextLeft, diffR = difficultyTextRight}
							local skillRow = T{961539551576, "<left><style PDARolloverTextDark><skillL></style><right><style PDARolloverTextBold><skillR></style>", skillL = skillTextLeft, skillR = skillTextRight}
						...
						PlaceObj('XTemplateWindow', {
							'__condition', function (parent, context) return context.errText end,
							'Margins', box(0, 5, 0, 0),
							'FoldWhenHidden', true,
							'Background', RGBA(32, 35, 47, 255),
						}, {
						...
						PlaceObj('XTemplateWindow', {
							'__condition', function (parent, context) return context.modifyMode end,
							'Id', "idButtonSection",
							'Margins', box(0, 5, 0, 0),
							'Padding', box(5, 1, 8, 1),
							'LayoutMethod', "HList",
							...
							PlaceObj('XTemplateTemplate', {
								'__template', "WeaponModToolbarButton",
								'Dock', "box",
								'OnPressEffect', "action",
								'OnPressParam', "actionModify",
								'Text', T(168008951549, --[[XTemplate WeaponModChoicePopup Text]] "Modify"),
							}),
							PlaceObj('XTemplateAction', {
								'ActionId', "actionModify",
								'ActionShortcut', "M",
								...
								'ActionState', function (self, host)
									...
									if not previewWindow or not previewWindow.context then return "hidden" end
				...
				PlaceObj('XTemplateWindow', {
					...
					PlaceObj('XTemplateWindow', {
						'comment', "empty (disabled for now)",
						'__condition', function (parent, context) return context.slot.CanBeEmpty end,
						'__class', "XButton",
						'RolloverAnchor', "top",
						'RolloverOffset', box(0, 0, 0, 20),
						'RolloverTitle', T(594069477550, --[[XTemplate WeaponModChoicePopup RolloverTitle]] "Empty"),
						'ZOrder', 0,
						'HAlign', "left",
						'VAlign', "top",
						'UseClipBox', false,
						...
						'FXPress', "WeaponModificationComponentClick",
						...
						'OnPress', function (self, gamepad)
							...
							if currentlyEquipped then itemId = false end
						...
						PlaceObj('XTemplateWindow', {
							...
							'MaxHeight', 90,
							'UseClipBox', false,
							'Image', "UI/Inventory/T_Backpack_Slot_Small",
							'ImageFit', "stretch",
						}),
						PlaceObj('XTemplateWindow', {
							'__class', "XImage",
						...
						PlaceObj('XTemplateWindow', {
							...
							'UseClipBox', false,
							'Transparency', 180,
							'Image', "UI/Icons/Operations/repair_item",
							'ImageScale', point(800, 800),
							'ImageColor', RGBA(215, 159, 80, 255),
						}),
					...
					PlaceObj('XTemplateForEach', {
						'comment', "slot alternatives",
						'array', function (parent, context) return context.slot.AvailableComponents end,
						'__context', function (parent, context, item, i, n) return SubContext(context, { item = IsKindOf(item , "WeaponColor") and item or WeaponComponents[item] }) end,
						...
						PlaceObj('XTemplateWindow', {
							...
							PlaceObj('XTemplateWindow', {
								'__class', "XImage",
								'Id', "idSpecialPartCost",
								'Margins', box(-5, -5, 0, 0),
								'HAlign', "left",
								'VAlign', "top",
								'UseClipBox', false,
								'Visible', false,
								'Image', "UI/Icons/Upgrades/parts_placeholder",
							}),
							PlaceObj('XTemplateWindow', {
								'Id', "idOverlay",
								'Background', RGBA(237, 31, 36, 25),
							}),
							PlaceObj('XTemplateWindow', {
								'Id', "idPartCost",
								'Margins', box(0, 0, 5, 0),
		...
		PlaceObj('XTemplateFunc', {
			'name', "ModifyPartPreview(self, partId, reason)",
			'func', function (self, partId, reason)
				if self.window_state == "destroying" then return end
				
				local partPreset = WeaponComponents[partId] or false
				if partId == "" then partPreset = partId end
				...
				-- In gamepad mode we want rollover to function like selection where possible
				...
				-- Prevent the default part from flickering when rolling over all the parts
				-- by delaying the visual revert
				...
				if ui then
					...
					if uiX < 0 then
						uiX = 0
					elseif uiX + previewUIWidth > listSize then
				...
				if not noErr and err == "blocked" then
					errText = T{742571156312, "<BlockedByError(param)>", param = errParam}
				end
				if InventoryIsCombatMode() then
					modifyMode = false
