...
SetpiecePrgs.GhostStories_FinalBattleIntro = function(seed, state, TriggerUnits)
	local li = { id = "GhostStories_FinalBattleIntro" }
	local rand = BraidRandomCreate(seed or AsyncRand())
	prgdbg(li, 1, 1) sprocall(SetpieceFadeOut.Exec, SetpieceFadeOut, state, rand, true, "", 0)
	prgdbg(li, 1, 2) sprocall(PrgPlayEffect.Exec, PrgPlayEffect, state, rand, false, "", {PlaceObj('ConditionalEffect', {'Conditions', {PlaceObj('SetpieceIsTestMode', {}),},'Effects', {PlaceObj('QuestSetVariableBool', {Prop = "SpawnThugs",QuestId = "GhostStories",}),},}),})
	prgdbg(li, 1, 3) sprocall(SetpieceFadeIn.Exec, SetpieceFadeIn, state, rand, false, "", 0, 800)
	local _, Thug1
	prgdbg(li, 1, 4) _, Thug1 = sprocall(SetpieceAssignFromGroup.Exec, SetpieceAssignFromGroup, state, rand, Thug1, "Thug1", "ThugActor1", "Object", false)
	local _, Thug2
	prgdbg(li, 1, 5) _, Thug2 = sprocall(SetpieceAssignFromGroup.Exec, SetpieceAssignFromGroup, state, rand, Thug2, "Thug2", "ThugActor2", "Object", false)
	local _, Thug3
	prgdbg(li, 1, 6) _, Thug3 = sprocall(SetpieceAssignFromGroup.Exec, SetpieceAssignFromGroup, state, rand, Thug3, "Thug3", "ThugActor3", "Object", false)
	local _, Thug1_Start
	prgdbg(li, 1, 7) _, Thug1_Start = sprocall(SetpieceTeleport.Exec, SetpieceTeleport, state, Thug1, "Thug1_Start", true)
	local _
	prgdbg(li, 1, 8) _, Thug1_Start = sprocall(SetpieceTeleport.Exec, SetpieceTeleport, state, Thug2, "Thug2_Start", true)
	local _
