...
SetpiecePrgs.PortCacaoFirstEnter_LegionActor1 = function(seed, state, MainActor)
	local li = { id = "PortCacaoFirstEnter_LegionActor1" }
	...
	prgdbg(li, 1, 7) _, RocketImpact = sprocall(SetpieceShoot.Exec, SetpieceShoot, state, rand, true, "", MainActor, "Point", nil, "Torso", "RocketImpact", 1, 0, 1000, 100, 0, 0)
	prgdbg(li, 1, 8) sprocall(SetpieceWaitCheckpoint.Exec, SetpieceWaitCheckpoint, state, rand, "PortCacaoCity_SecondBanterDone")
	prgdbg(li, 1, 9) sprocall(SetpieceSleep.Exec, SetpieceSleep, state, rand, true, "", 200)
end