-- ========== GENERATED BY ParticleSystemPreset Editor DO NOT EDIT MANUALLY! ==========

PlaceObj('ParticleSystemPreset', {
	game_time_animated = true,
	group = "Trap_Electrical",
	id = "Explosion_Electrical",
	PlaceObj('ParticleEmitter', {
		'label', "Spkars",
		'time_start', 200,
		'time_stop', 500,
		'emit_detail_level', 100,
		'max_live_count', 30,
		'parts_per_sec', 6000,
		'lifetime_min', 400,
		'lifetime_max', 1200,
		'position', point(0, 0, 600),
		'shader', "Add",
		'texture', "Textures/Particles/white.tga",
		'self_illum', 100,
		'outlines', {
			{
				point(248, 3832),
				point(3832, 3832),
				point(3832, 248),
				point(248, 248),
			},
		},
		'texture_hash', 2617278910886611064,
	}, nil, nil),
	PlaceObj('ParticleEmitter', {
		'label', "Spkars",
		'time_start', 1400,
		'time_stop', 1600,
		'emit_detail_level', 100,
		'max_live_count', 15,
		'parts_per_sec', 6000,
		'lifetime_min', 400,
		'lifetime_max', 1000,
		'position', point(0, 0, 600),
		'shader', "Add",
		'texture', "Textures/Particles/white.tga",
		'self_illum', 100,
		'outlines', {
			{
				point(248, 3832),
				point(3832, 3832),
				point(3832, 248),
				point(248, 248),
			},
		},
		'texture_hash', 2617278910886611064,
	}, nil, nil),
	PlaceObj('ParticleBehaviorEmissive', {
		'bins', set( "A", "C" ),
		'emissive_curve', {
			range_y = 10,
			scale = 1000,
			point(0, 1000, 1000),
			point(259, 821, 821),
			point(769, 379, 379),
			point(1000, 90, 90),
		},
	}, nil, nil),
	PlaceObj('ParticleBehaviorColorize', {
		'start_color_min', RGBA(255, 246, 198, 255),
		'start_color_max', RGBA(254, 211, 121, 255),
		'mid_color', RGBA(245, 125, 45, 255),
		'end_color', RGBA(196, 63, 36, 255),
	}, nil, nil),
	PlaceObj('ParticleBehaviorFadeInOut', {
		'fade_curve', {
			range_y = 10,
			scale = 1000,
			point(0, 0, 0),
			point(204, 121, 805),
			point(642, 98, 344),
			point(1000, 0, 0),
		},
	}, nil, nil),
	PlaceObj('ParticleBehaviorResizeCurve', {
		'max_size', 400,
		'resize_type', "velocity",
		'size_curve', {
			range_y = 10,
			scale = 1000,
			point(0, 17, 17),
			point(338, 37, 37),
			point(741, 32, 32),
			point(1000, 0, 0),
		},
		'non_square_size', true,
		'size_curve_2', {
			range_y = 10,
			scale = 1000,
			point(0, 0, 0),
			point(470, 384, 737),
			point(753, 118, 118),
			point(1000, 0, 0),
		},
	}, nil, nil),
	PlaceObj('DisplacerSphere', {
		'inner_radius', 1,
		'outer_radius', 100,
	}, nil, nil),
	PlaceObj('ParticleBehaviorFriction', {
		'probability', 50,
		'friction', {
			range_y = 10,
			scale = 1000,
			point(0, 973, 1059),
			point(310, 884, 884),
			point(662, 884, 884),
			point(1000, 891, 891),
		},
	}, nil, nil),
	PlaceObj('FaceAlongMovement', nil, nil, nil),
	PlaceObj('ParticleBehaviorRandomSpeedSpray', {
		'spread_angle', 18000,
		'vel_max', 3200,
	}, nil, nil),
	PlaceObj('ParticleBehaviorGravityWind', {
		'direction', point(0, 0, -1000),
		'acceleration', 2000,
		'max_vel', 4000,
	}, nil, nil),
	PlaceObj('ParticleEmitter', {
		'label', "Smoke",
		'bins', set( "B" ),
		'time_stop', 2000,
		'world_space', true,
		'emit_detail_level', 100,
		'max_live_count', 60,
		'parts_per_sec', 6000,
		'lifetime_min', 2000,
		'lifetime_max', 4000,
		'position', point(0, 0, 600),
		'angle', range(0, 45),
		'size_max', 3000,
		'shader', "Add Light",
		'texture', "Textures/Particles/Clouds_1024.tga",
		'normal_as_flow_map', true,
		'normalmap', "Textures/Particles/Clouds_1024.norm.tga",
		'frames', point(4, 4),
		'light_softness', 1000,
		'flow_speed', 103,
		'flow_scale', 55,
		'softness', 20,
		'outlines', {
			{
				point(144, 768),
				point(620, 1020),
				point(1020, 288),
				point(284, 12),
			},
			{
				point(1072, 324),
				point(1072, 792),
				point(2008, 964),
				point(1844, 68),
			},
			{
				point(2064, 800),
				point(2912, 944),
				point(2932, 328),
				point(2256, 24),
			},
			{
				point(3300, 936),
				point(4060, 936),
				point(3848, 124),
				point(3088, 124),
			},
			{
				point(88, 1232),
				point(180, 1904),
				point(824, 2032),
				point(824, 1164),
			},
			{
				point(1072, 1480),
				point(1680, 2008),
				point(1900, 1468),
				point(1596, 1192),
			},
			{
				point(2156, 1284),
				point(2156, 2044),
				point(2980, 2044),
				point(2796, 1240),
			},
			{
				point(3140, 1872),
				point(3852, 2028),
				point(3896, 1272),
				point(3140, 1180),
			},
			{
				point(196, 3064),
				point(908, 2828),
				point(972, 2264),
				point(80, 2100),
			},
			{
				point(1228, 2172),
				point(1156, 3044),
				point(1940, 3008),
				point(1940, 2056),
			},
			{
				point(2076, 2180),
				point(2076, 3004),
				point(2948, 3004),
				point(2948, 2180),
			},
			{
				point(3220, 2896),
				point(3972, 3032),
				point(3768, 2228),
				point(3100, 2204),
			},
			{
				point(116, 4004),
				point(884, 3908),
				point(920, 3208),
				point(96, 3208),
			},
			{
				point(1036, 3688),
				point(1960, 3992),
				point(1888, 3216),
				point(1432, 3136),
			},
			{
				point(2068, 3104),
				point(2068, 3908),
				point(3068, 3972),
				point(3068, 3076),
			},
			{
				point(3132, 3936),
				point(4052, 3936),
				point(4052, 3148),
				point(3132, 3148),
			},
		},
		'texture_hash', -3185377314160643337,
	}, nil, nil),
	PlaceObj('ParticleBehaviorPickFrame', {
		'bins', set( "B" ),
	}, nil, nil),
	PlaceObj('ParticleBehaviorResizeCurve', {
		'bins', set( "B" ),
		'max_size', 2000,
		'size_curve', {
			range_y = 10,
			scale = 1000,
			point(0, 183, 183),
			point(354, 263, 263),
			point(732, 343, 343),
			point(1000, 371, 371),
		},
	}, nil, nil),
	PlaceObj('ParticleBehaviorColorize', {
		'bins', set( "B" ),
		'start_color_min', RGBA(195, 199, 203, 255),
		'start_color_max', RGBA(151, 163, 175, 255),
		'mid_color', RGBA(75, 80, 87, 255),
		'mid_intensity', 2752,
		'end_color', RGBA(176, 211, 224, 255),
		'type', "One of four",
		'color_curve', {
			range_y = 10,
			scale = 1000,
			point(0, 0, 0),
			point(333, 667, 667),
			point(667, 1333, 1333),
			point(1000, 2000, 2000),
		},
	}, nil, nil),
	PlaceObj('ParticleBehaviorFadeInOut', {
		'bins', set( "B" ),
		'fade_curve', {
			range_y = 10,
			scale = 1000,
			point(0, 0, 0),
			point(279, 51, 111),
			point(761, 23, 63),
			point(1000, 0, 0),
		},
	}, nil, nil),
	PlaceObj('ParticleBehaviorGravityWind', {
		'bins', set( "B" ),
		'start_vel', 100,
		'acceleration', 300,
		'max_vel', 500,
	}, nil, nil),
	PlaceObj('ParticleBehaviorWind', {
		'bins', set( "B" ),
		'multiplier', 400,
	}, nil, nil),
	PlaceObj('ParticleBehaviorFriction', {
		'bins', set( "B" ),
		'friction', {
			range_y = 10,
			scale = 1000,
			point(0, 992, 992),
			point(331, 981, 981),
			point(667, 974, 974),
			point(1000, 970, 970),
		},
	}, nil, nil),
	PlaceObj('ParticleBehaviorRotate', {
		'bins', set( "B" ),
		'probability', 50,
		'rpm_curve', {
			range_y = 10,
			scale = 10,
			point(0, 13, 13),
			point(325, 17, 17),
			point(679, 26, 26),
			point(1000, 0, 0),
		},
		'rpm_curve_range', range(-200, 200),
	}, nil, nil),
	PlaceObj('ParticleEmitter', {
		'label', "Small lightnings",
		'bins', set( "C" ),
		'time_stop', 500,
		'emit_detail_level', 100,
		'max_live_count', 120,
		'parts_per_sec', 12000,
		'lifetime_min', 20,
		'lifetime_max', 40,
		'position', point(0, 0, 600),
		'angle', range(0, 360),
		'shader', "Add",
		'texture', "Textures/Particles/Lightning_A.tga",
		'frames', point(2, 2),
		'self_illum', 100,
		'softness', 20,
		'alpha', range(0, 255),
		'outlines', {
			{
				point(256, 368),
				point(240, 1504),
				point(1616, 1488),
				point(864, 320),
			},
			{
				point(2336, 544),
				point(2672, 1632),
				point(3952, 912),
				point(3392, 240),
			},
			{
				point(496, 2352),
				point(560, 3680),
				point(1104, 3408),
				point(1424, 2208),
			},
			{
				point(2128, 2976),
				point(2864, 3792),
				point(3824, 3440),
				point(3824, 2144),
			},
		},
		'texture_hash', -7183221940945445393,
	}, nil, nil),
	PlaceObj('ParticleEmitter', {
		'label', "Small lightnings",
		'bins', set( "C" ),
		'time_start', 1200,
		'time_stop', 1500,
		'emit_detail_level', 100,
		'max_live_count', 120,
		'parts_per_sec', 12000,
		'lifetime_min', 20,
		'lifetime_max', 40,
		'position', point(0, 0, 600),
		'angle', range(0, 360),
		'shader', "Add",
		'texture', "Textures/Particles/Lightning_A.tga",
		'frames', point(2, 2),
		'self_illum', 100,
		'softness', 20,
		'alpha', range(0, 255),
		'outlines', {
			{
				point(256, 368),
				point(240, 1504),
				point(1616, 1488),
				point(864, 320),
			},
			{
				point(2336, 544),
				point(2672, 1632),
				point(3952, 912),
				point(3392, 240),
			},
			{
				point(496, 2352),
				point(560, 3680),
				point(1104, 3408),
				point(1424, 2208),
			},
			{
				point(2128, 2976),
				point(2864, 3792),
				point(3824, 3440),
				point(3824, 2144),
			},
		},
		'texture_hash', -7183221940945445393,
	}, nil, nil),
	PlaceObj('DisplacerLine', {
		'bins', set( "B", "C" ),
		'position1', point(-300, 0, 0),
		'position2', point(300, 0, 0),
	}, nil, nil),
	PlaceObj('DisplacerLine', {
		'bins', set( "B", "C", "E" ),
		'position1', point(0, 0, -300),
		'position2', point(0, 0, 300),
	}, nil, nil),
	PlaceObj('ParticleBehaviorPickFrame', {
		'bins', set( "C" ),
	}, nil, nil),
	PlaceObj('ParticleBehaviorColorize', {
		'bins', set( "C" ),
		'start_color_min', RGBA(255, 255, 255, 255),
		'start_color_max', RGBA(255, 241, 202, 255),
		'mid_color', RGBA(196, 63, 36, 255),
		'end_color', RGBA(196, 63, 36, 255),
		'type', "Start color range",
	}, nil, nil),
	PlaceObj('ParticleBehaviorResizeCurve', {
		'bins', set( "C" ),
		'max_size', 2000,
		'size_curve', {
			range_y = 10,
			scale = 1000,
			point(0, 11, 11),
			point(66, 251, 251),
			point(928, 257, 257),
			point(1000, 229, 229),
		},
	}, nil, nil),
	PlaceObj('ParticleEmitter', {
		'label', "lightnings",
		'bins', set( "E" ),
		'time_stop', 500,
		'emit_detail_level', 100,
		'max_live_count', 60,
		'parts_per_sec', 3000,
		'lifetime_min', 20,
		'lifetime_max', 100,
		'position', point(300, 0, 600),
		'shader', "Add",
		'texture', "Textures/Particles/Lightning_A.tga",
		'frames', point(2, 2),
		'self_illum', 200,
		'softness', 20,
		'alpha', range(0, 255),
		'outlines', {
			{
				point(256, 368),
				point(240, 1504),
				point(1616, 1488),
				point(864, 320),
			},
			{
				point(2336, 544),
				point(2672, 1632),
				point(3952, 912),
				point(3392, 240),
			},
			{
				point(496, 2352),
				point(560, 3680),
				point(1104, 3408),
				point(1424, 2208),
			},
			{
				point(2128, 2976),
				point(2864, 3792),
				point(3824, 3440),
				point(3824, 2144),
			},
		},
		'texture_hash', -7183221940945445393,
	}, nil, nil),
	PlaceObj('FaceAlongMovement', {
		'bins', set( "E" ),
		'rotate', true,
	}, nil, nil),
	PlaceObj('ParticleBehaviorAnimate', {
		'bins', set( "E" ),
		'anim_type', "Ping-Pong",
		'fps', 30,
		'sequence_time_remap', {
			range_y = 10,
			scale = 1000,
			point(0, 0, 0),
			point(333, 333, 333),
			point(667, 667, 667),
			point(1000, 1000, 1000),
		},
	}, nil, nil),
	PlaceObj('DisplacerLine', {
		'bins', set( "E" ),
		'position1', point(0, -300, 0),
		'position2', point(0, 300, 0),
	}, nil, nil),
	PlaceObj('ParticleBehaviorGravityWell', {
		'bins', set( "E" ),
		'position', point(1000, 0, 600),
		'start_vel', 1000,
		'acceleration', 1000,
	}, nil, nil),
	PlaceObj('ParticleBehaviorResizeCurve', {
		'bins', set( "E" ),
		'max_size', 1000,
		'size_curve', {
			range_y = 10,
			scale = 1000,
			point(0, 237, 526),
			point(323, 253, 511),
			point(663, 247, 495),
			point(1000, 263, 495),
		},
		'non_square_size', true,
		'max_size_2', 3000,
		'size_curve_2', {
			range_y = 10,
			scale = 1000,
			point(0, 742, 742),
			point(274, 732, 732),
			point(628, 653, 757),
			point(1000, 570, 746),
		},
	}, nil, nil),
})

